...
void Jump::exec(State &state) { state.registers[pc] = (state.labels[label] - 1) * 32; }
...
void JumpAndLink::exec(State &state) {
  state.registers[return_register] = state.registers[pc] * 32; // May be plus 1
  state.registers[pc] = (state.labels[label] - 1) * 32;
}
